...
export default function HomePage() {
  ...
  return (
    <main className="market-shell min-h-[100dvh]">
      ...
      <section className="bg-white py-12" id="services">
        <div className="market-container"><div className="mb-6 max-w-[650px]"><p className="font-label text-[9px] uppercase tracking-[.18em] text-[#4f6bff]">More than a checkout</p><h2 className="mt-2 text-3xl font-extrabold tracking-[-.06em]">Your piece, your measurements.</h2><p className="mt-3 text-[13px] leading-6 text-[#696663]">From a quick alteration to a fully made-to-measure bridal blouse, our stylists help you land the fit before the fabric hits the table.</p></div>
          <div className="grid gap-3 sm:grid-cols-3">
            {[[Ruler, 'Custom measurements', 'A fitting that starts with how you move.'], [Scissors, 'Alterations', 'Small changes, big difference.'], [Sparkles, 'Aari & bridal', 'Handwork designed around your story.']].map(([Icon, title, text]) => { const ServiceIcon = Icon as typeof Ruler; return <div key={title as string} className="rounded-xl border border-[#e8e4df] bg-[#faf8f5] p-5"><ServiceIcon size={19} className="text-[#4f6bff]" /><h3 className="mt-4 text-[13px] font-bold">{title as string}</h3><p className="mt-2 text-[11px] leading-5 text-[#77736f]">{text as string}</p><a href="mailto:hello@mahathitailor.in?subject=Tailoring%20appointment" className="mt-4 inline-flex items-center gap-1 text-[10px] font-bold text-[#4f6bff]">Book now <ArrowRight size={12} /></a></div> })}
          </div>
        </div>
      </section>
      ...
      <section id="appointment" className="market-container pb-14"><div className="gradient-ink flex flex-col justify-between gap-6 rounded-2xl p-7 text-white sm:flex-row sm:items-center sm:p-9"><div><p className="font-label text-[9px] uppercase tracking-[.18em] text-white/70">In Hyderabad? Come by.</p><h2 className="mt-2 text-2xl font-extrabold tracking-[-.05em]">Let&apos;s find your right fit.</h2><p className="mt-2 text-[12px] text-white/75">Appointments available Tuesday–Sunday, 11am–7pm.</p></div><a href="mailto:hello@mahathitailor.in?subject=Book%20a%20Mahathi%20fitting" className="inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-lg bg-white px-5 text-[11px] font-bold text-[#4f6bff]"><CalendarDays size={15} /> Book an appointment</a></div></section>
